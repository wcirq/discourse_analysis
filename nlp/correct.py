...
def get_candidates(error_phrase):
    ...
    re.sub("-", "/", error_pinyin)
    cn_words_dict = load_cn_words_dict(os.path.join(DATA_PATH, "cn_dict.txt"))
...
def auto_correct_sentence(error_sentence, verbose=True, words=None):
    if words is None:
        words = phrase_freq.keys()
    else:
        words.extend(phrase_freq.keys())
